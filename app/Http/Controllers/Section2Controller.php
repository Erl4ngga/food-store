...
class Section2Controller extends Controller
{
    ...
    {
        $section=Section2::orderBy('id','DESC')->paginate(10);
        return view('backend.section2.index')->with('section',$section);
    }
        /**
    ...
    {
        $section=Section2::find($id);
        if(!$section){
            request()->session()->flash('error','section not found');
        }
        return view('backend.section2.edit')->with('section',$section);
    }
    /**
